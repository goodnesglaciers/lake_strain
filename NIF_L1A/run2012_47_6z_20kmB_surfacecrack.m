%% RUNFILE FOR 2012 NIF
%  November 20 2020: run the 2012 NIF with all stations and a 20km by 20 km basal patch.
%  Use with makegeom_caseC_20km.m for 20km basal plane + L1A hydro-fracture
%  Runs NIF and saves output as: MLE2012_47_range7_gc_20km_surfacecrack.mat
...
% preferably already downsampled in time to make this go quicker.
         ...
         load ts_displ_2012_154_168_oct16.mat
       
         tstart=159.0; 
         tend=163.0; % event at DOY 161.8
         [a,I1]=min(abs(ts_displ_2012_154_168.epochs-tstart));
         [a,I2]=min(abs(ts_displ_2012_154_168.epochs-tend));

         
% Select epochs by subsetting (from 1 down to 3)
         ts2=ts_displ_2012_154_168(:,I1:3:I2);
...
ts_enu=tsb_enu;   %allready subsetted above
...
if(dosetup)
  ...
  [G1, G2, G3, G2b, G3b, Rc, Rb, patchesC, patchesB]=makegeom_caseC_20km_2012_surfacecrack(ts_enu,origin); %% change to 20 km bed
...
tstart=159.3;  % start and stop times of tso for background vels
tend=160.5;
  ...
  for j=1:Nsites
    for k=1:3
      if(length(ts_enu(j,emin:emax).d(:))>=10)
        ...
        X=ts_enu(j,emin:emax).epochs(:)-ref_epoch;
...
% PRIORS 
     ...
     % LOOSE CONSTRAINT FOR FRAME ERROR
      for j=2*Nbasis+Ncomps+1:statedim
        ...
        alphaC=250; %linspace(3000,10000, 3); % temporal smoothing 
        gammaC=450; %linspace(5, 35, 3);
        alphaB=50; %linspace(1000, 10000, 3);  %20 is good for 8 by 4
        gammaB=25; %linspace(5, 55, 3); % spatial smoothing
...
% set up hyperparameters to loop through        
        ...
        val = zeros(ntheta2C, ntheta2B, ntheta3B, ntheta1);
        sig2 = zeros(ntheta2C, ntheta2B, ntheta3B, ntheta1);

% % %%%%%%%%%%%%%%%%%%%%% NOW RUN FILTER %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%        

for iii=1 % individual run for each value in fuxi
%    for jjj=1
     %   for kkk=1:4
        %    for lll=1:4
[val(iii), sig2(iii), x_s, xcov] = ...
	  netfilt_withvert_MLEiii14(ts_enu, Kern, x0, var0, sitesV, X0_enu, V0_enu, ...
      V0_cov_enu, ref_epoch, origin, R1, R2, smooth, c, sig_f,...
      theta2C, theta3C(iii), theta2B, theta3B, theta1)

save val_47_range7_gc_20km val
save sig2_47_range7_gc_20km sig2
...
save MLE2012_47_range7_gc_20km_surfacecrack.mat
% % % % %
disp('Done')       
