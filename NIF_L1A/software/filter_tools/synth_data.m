    function Drand = synth_data(t, sites, files, origin, Signal, tau, sigma);

%%           Drand = synth_data(t, sites, files, origin, Signal, tau, sigma);
%%
...
% Next order the data and add spatially correlated white noise w/ variance sigma^2
...
%%	Dat(1:3*Nstsob ,k) = ENU;
	...
	Drand(1:3*Nstsob ,k) = ENU_pred + sigma*Gamma*randn(size(ENU));
