...
% 2023 May 31 LAS -- for 2012 drainage
clear all; close all;

%% 2012 NIF L1A outputs
...
load n2012_allfour_surfC.mat % all 3 sources + winter = "ALLFOUR"

% calculate \sigma_{2} within lake basins
for i=1:length(time_2012)
    % NL n2012
    n2012_allfour.princ_sigma2_NL(i,1) = nanmean(nanmean(n2012_allfour.sigma2(i,indNL_surf)));
    % NNL n2012
    n2012_allfour.princ_sigma2_NNL(i,1) = nanmean(nanmean(n2012_allfour.sigma2(i,indNNL_surf)));
    % SNL n2012
    n2012_allfour.princ_sigma2_SNL(i,1) = nanmean(nanmean(n2012_allfour.sigma2(i,indSNL_surf)));
end
...
for i=160:1:736 %:length(time_2012)
fig1 = figure('Units','centimeters','Position',1.2.*[40 1 20.5 16]); clf;
    % mapview
...
% tick marks 
        hypot = 0.2;
% lake + winter P2
        theta_15GPa_P2_plotX1(1,:) = xy_surf_fine(:,1) + hypot.*(reshape(cosd(rad2deg(n2012_allfour.theta(i,:))+90),81*81,1));  
        theta_15GPa_P2_plotX2(1,:) = xy_surf_fine(:,1) - hypot.*(reshape(cosd(rad2deg(n2012_allfour.theta(i,:))+90),81*81,1)); 
        theta_15GPa_P2_plotY1(1,:) = xy_surf_fine(:,2) + hypot.*(reshape(sind(rad2deg(n2012_allfour.theta(i,:))+90),81*81,1)); 
        theta_15GPa_P2_plotY2(1,:) = xy_surf_fine(:,2) - hypot.*(reshape(sind(rad2deg(n2012_allfour.theta(i,:))+90),81*81,1)); 
...
text(-9.1, 6.65, '{\itStevens et al.} (2024) Movie M2','FontSize',9,'FontName','Avenir');
...
text(5.5,6.6,sprintf('2012/%6.3f',time_2012(i)),'FontWeight','bold','FontSize',10);

axes(axe2) % principle stress \sigma_{2} -- all 3 sources + winter = ALLFOUR
hold on;
[C2,h2]=contourf(xx_fine,yy_fine,squeeze(n2012_allfour.sigma2(i,:,:)./1e3),vvKPA);
...
% tick marks 
        hypot = 0.2;
% lake + winter P2
        theta_15GPa_P2_plotX1(1,:) = xy_surf_fine(:,1) + hypot.*(reshape(cosd(rad2deg(n2012_allfour.theta(i,:))),81*81,1));  
        theta_15GPa_P2_plotX2(1,:) = xy_surf_fine(:,1) - hypot.*(reshape(cosd(rad2deg(n2012_allfour.theta(i,:))),81*81,1)); 
        theta_15GPa_P2_plotY1(1,:) = xy_surf_fine(:,2) + hypot.*(reshape(sind(rad2deg(n2012_allfour.theta(i,:))),81*81,1)); 
        theta_15GPa_P2_plotY2(1,:) = xy_surf_fine(:,2) - hypot.*(reshape(sind(rad2deg(n2012_allfour.theta(i,:))),81*81,1)); 
%theta_P tick marks winter + lake
    for j=2200:1:4400
    plot([theta_15GPa_P2_plotX2(1,j) theta_15GPa_P2_plotX1(1,j)],...
        [theta_15GPa_P2_plotY2(1,j) theta_15GPa_P2_plotY1(1,j)],...
...
set(t2,'YTick',[-1000,-500,0,500,1000],'TickDirection','out'); 
hold all
...
plot(time_2012(160:736),(n2012_allfour.princ_sigma1_NNL(160:736))./1e3,'-','LineWidth',1.2,'Color',metal); 
plot(time_2012(160:736),(n2012_allfour.princ_sigma1_NL(160:736))./1e3,'-','LineWidth',1.2,'Color',handle); 
plot(time_2012(160:736),(n2012_allfour.princ_sigma1_SNL(160:736))./1e3,'-','LineWidth',1.2,'Color',sky_blue); 
...
text(161.20-0.05,1200,'{\itt}_{1}','Rotation',0,'FontSize',8,'FontName','Avenir'); 
text(161.72-0.05,1200,'{\itt}_{2}','Rotation',0,'FontSize',8,'FontName','Avenir'); 
text(161.85-0.05,1200,'{\itt}_{3}','FontSize',8,'FontName','Avenir'); 
text(162.85-0.05,1200,'{\itt}_{4}','Rotation',0,'FontSize',8,'FontName','Avenir'); 
text(161.85-0.07+(4/24),1200,'{\it\tau} = {6 hr}','Rotation',0,'FontSize',8,'FontName','Avenir','Color',[0.2 0.55 0.2]); 
...
% moving time marker
plot([time_2012(i) time_2012(i)],[-2000 1200],'-','LineWidth',1.05,'Color',[0.4940 0.1840 0.5560]);
plot(time_2012(i),1000,'v','MarkerSize',TriangleSize-3,'MarkerFaceColor',[0.4940 0.1840 0.5560],'Color',[0.4940 0.1840 0.5560]);

xlabel('Day of Year, 2012  [ UTC ]'); ylabel('\sigma_{1} [ kPa ]');
lgd_s1=legend('L1C','L1A','L1B');
legend boxoff
set(lgd_s1,'Position',[0.1 0.36 0.1 0.03]);
ylim([-500 1000]); xlim([162.85-3 162.85]);
text(159.15, 1200, 'c','FontSize',10,'FontWeight','bold');
text(159.15, -1400, 'e','FontSize',10,'FontWeight','bold');
...
plot(time_2012(160:1:736),(n2012_allfour.princ_sigma2_NNL(160:1:736))./1e3,'-','LineWidth',1.2,'Color',metal); 
plot(time_2012(160:1:736),(n2012_allfour.princ_sigma2_NL(160:1:736))./1e3,'-','LineWidth',1.2,'Color',handle); 
plot(time_2012(160:1:736),(n2012_allfour.princ_sigma2_SNL(160:1:736))./1e3,'-','LineWidth',1.2,'Color',sky_blue); 
...
plot(time_2012(160:736),(n2012_allfour.princ_sigma2_NNL(160:736))./1e3,'-','LineWidth',1.2,'Color',metal); 
plot(time_2012(160:736),(n2012_allfour.princ_sigma2_SNL(160:736))./1e3,'-','LineWidth',1.2,'Color',sky_blue); 
plot(time_2012(160:736),(n2012_allfour.princ_sigma2_NL(160:736))./1e3,'-','LineWidth',1.2,'Color',handle); 

% moving time marker
plot([time_2012(i) time_2012(i)],[-2000 1200],'-','LineWidth',1.05,'Color',[0.4940 0.1840 0.5560]);
plot(time_2012(i),1000,'v','MarkerSize',TriangleSize-3,'MarkerFaceColor',[0.4940 0.1840 0.5560],'Color',[0.4940 0.1840 0.5560]);

xlabel('Day of Year, 2012  [ UTC ]'); ylabel('\sigma_{2} [ kPa ]');
...
set(gca,'FontName','Avenir','ytick',[-500,0,500,1000]);
ylim([-500 1000]); xlim([162.85-3 162.85]);
text(159.55, 1200, 'd','FontSize',10,'FontWeight','bold');
text(159.55, -1400, 'f','FontSize',10,'FontWeight','bold');
grid on 
...
tt_NNL = rad2deg(n2012_allfour.theta(i,indNNL_surf1))+90;
...
tt = rad2deg(n2012_allfour.theta(i,indNL_surf1))+90;
...
polarhistogram(deg2rad(theta_SNL_winter(1,2)-90),'BinWidth',pi./33,'Normalization','probability',...
    'FaceColor',[0.6 0.6 0.6],'FaceAlpha',.7,'EdgeColor',[0.1 0.1 0.1]);
% call values; assign color
tt_SNL = rad2deg(n2012_allfour.theta(i,indSNL_surf1))+90;
...
polarhistogram(deg2rad(theta_NNL_winter(2,3)+180),'BinWidth',pi./33,'Normalization','probability',...
    'FaceColor',[0.6 0.6 0.6],'FaceAlpha',.7,'EdgeColor',[0.1 0.1 0.1]);
% % call values; select location on HF; assign color; 
tt_NNL = rad2deg(n2012_allfour.theta(i,indNNL_surf1));
color = round(n2012_allfour.princ_sigma2_NNL(i)./1e3);
index = fix((color-cmin)/(cmax-cmin)*m)+1;
RGB = ind2rgb(index,polar_cmap);
% plot
polarhistogram(deg2rad(tt_NNL),'BinWidth',pi./23,'Normalization','probability',...
...
polarhistogram(deg2rad(theta_NL_winter(1,4)+180),'BinWidth',pi./33,'Normalization','probability',...
    'FaceColor',[0.6 0.6 0.6],'FaceAlpha',.7,'EdgeColor',[0.1 0.1 0.1]);
% call values; assign color
tt = rad2deg(n2012_allfour.theta(i,indNL_surf1));
color = round(n2012_allfour.princ_sigma2_NL(i)./1e3);
index = fix((color-cmin)/(cmax-cmin)*m)+1;
RGB = ind2rgb(index,polar_cmap);
% plot
polarhistogram(deg2rad(tt),'BinWidth',pi./30,'Normalization','probability',...
...
polarhistogram(deg2rad(theta_SNL_winter(1,2)+180),'BinWidth',pi./33,'Normalization','probability',...
    'FaceColor',[0.6 0.6 0.6],'FaceAlpha',.7,'EdgeColor',[0.1 0.1 0.1]);
% call values; assign color
tt_SNL = rad2deg(n2012_allfour.theta(i,indSNL_surf1));
color = round(n2012_allfour.princ_sigma2_SNL(i)./1e3);
index = fix((color-cmin)/(cmax-cmin)*m)+1;
RGB = ind2rgb(index,polar_cmap);
% plot
polarhistogram(deg2rad(tt_SNL),'BinWidth',pi./30,'Normalization','probability',...
...
path='Animation2012_dials_2024Feb'; % folder path
name=sprintf('img%04d.png',i-158); % file name
