%% Nsurface principle stress paper figure for 2012
...
%% 2012 saved NIF outputs and forward-modeled stresses
load('../NIF_L1A/MLE2012_47_range7_gc_20km_surfacecrack.mat'); % NIF outputs for 20 km bed; L1A set-up.
...
load n2012_allfour_surfC.mat % 2012
...
load('out2012_1day_20km_third.mat') % GPS displacements at precursor, max open, 1-day
load apcoords_lle_2012.mat
...
load lil_lake_2013_168.mat % SNL
...
% colorbar ranges
cmin_vert = -0.5; cmax_vert = 0.5; % vertical crack
...
% lakes
scatter(xy_lake(:,1),xy_lake(:,2),5,'bo','filled','MarkerEdgeColor','none')
scatter(xy_lil_lake(:,1),xy_lil_lake(:,2),5,'bo','filled','MarkerEdgeColor','none')
scatter(xy_nnl_lake(:,1),xy_nnl_lake(:,2),5,'bo','filled','MarkerEdgeColor','none')
plot(xy_sta_12(:,1),xy_sta_12(:,2),'k^','MarkerSize',TriangleSize-1,'MarkerFaceColor','none');
load('V0_enu_2012.mat'); % pre-drainage GPS velocities from ENU displacements
v_east = V0_enu(1:3:end); v_north = V0_enu(2:3:end);
quiver(xy_sta_12(1:16,1), xy_sta_12(1:16,2),...
...
hold on;
load out2012_20kmB_surfC.mat
z=out2012_20kmB_surfC.vert_crack_open_m;
 Nx=24;
 Nz=6; 
 isf=0;
 for i=1:Nx
 ...
 caxis([-0.5 0.5]); 
 set(gca,'ydir','rev');  
 xlim([-2.01 2.51]); ylim([0.09 1.11]);
 set(gca,'FontSize',m-1)
text(-2.8, 0.2, 'b','FontWeight','bold','FontSize',13);
% 1 km scale bar
...
tt21=colorbar('EastOutside'); set(tt21,'YTick',[-0.5, 0, 0.5]); hold all
...
hold on;
z=-1.*out2012_20kmB_surfC.bed_crack_slip_m;
 Nx=24;
 Ny=24; 
 isf=0;
 for i=1:Nx
  for j=1:Ny
    ...
    x1 = patchesB(isf,1);
    x2 = x1 - 0.833333333; % less patch width
    y1 = patchesB(isf,2);
    y2 = y1 - 0.833333333; % less patch width
    x=[x1, x2, x2, x1];
    y=[y1, y1, y2, y2];
    patch(x,y,z(isf),'EdgeColor',[0.6 0.6 0.6]); hold on;
...
plot(patchesC(1:6:end,6),patchesC(1:6:end,7),'y','LineWidth',2)
quiver(xy_sta_12(1:16,1), xy_sta_12(1:16,2),...
    out2012.u_opening_GPS'+out2012.u_precursor_GPS',...
    out2012.v_opening_GPS'+out2012.v_precursor_GPS',...
...
xlim([-10 10]); ylim([-10 10]);
text(-13.5, 9.5, 'c','FontWeight','bold','FontSize',13);
axis off
...
hold on;
z=out2012_20kmB_surfC.bed_crack_open_m;
 Nx=24;
 Ny=24; 
 isf=0;
...
text(-9.366666666,7.7,'N','FontSize',11)
xlim([-10 10]); ylim([-10 10]);
text(-13.5, 9.5, 'd','FontWeight','bold','FontSize',13);
axis off
...
hold on; text(-13.5, 13.5, 'e','FontWeight','bold','FontSize',12);
...
plot(xy_sta_12(:,1),xy_sta_12(:,2),'k^','MarkerSize',TriangleSize-2,'MarkerFaceColor','k'); hold on;
...
hold on; 
i=544;
[~,h6]=contourf(xx_fine,yy_fine,(squeeze(n2012_allfour.sigma1(i,:,:))-princ_sigma1_winter_vq)./1e3,v); 
set(h6,'LineColor','none'); 
contour(xx_fine,yy_fine,(squeeze(n2012_allfour.sigma1(i,:,:))-princ_sigma1_winter_vq)./1e3,[v200 v200],'k','LineWidth',1.1);
contour(xx_fine,yy_fine,(squeeze(n2012_allfour.sigma1_39mu(i,:,:))-princ_sigma1_winter_vq)./1e3,[v200 v200],'k--','LineWidth',0.8);
contour(xx_fine,yy_fine,(squeeze(n2012_allfour.sigma1_032mu(i,:,:))-princ_sigma1_winter_vq)./1e3,[v200 v200],'w','LineWidth',1.5);

% % calculate r_m of 200 kPa contour across \lambda and \mu values
% %dmax 2012 all quadrants
% [Cdmax,~]=contour(xx_fine,yy_fine,(squeeze(n2012_allfour.sigma1(i,:,:))-princ_sigma1_winter_vq)./1e3,[v200 v200]); 
% Cdmax(Cdmax > 15) = NaN;
% NLdmax2012_mu15 = nanmax(sqrt((Cdmax(1,2:end).^2)+(Cdmax(2,2:end).^2)))  
% 
% % % %dmax 2012 inland
% [Cdmax,~]=contour(xx_fine,yy_fine,(squeeze(n2012_allfour.sigma1(i,:,:))-princ_sigma1_winter_vq)./1e3,[v200 v200]); 
% Cdmax(Cdmax > 15) = NaN;
% test = Cdmax(1,:); test(test < 0.5) = NaN;
% NLdmax2012_mu15_inland = nanmax(sqrt((test(1,2:end).^2)+(Cdmax(2,2:end).^2)))  
%  
% contour(xx_fine,yy_fine,(squeeze(n2012_allfour.sigma1_39mu(i,:,:))-princ_sigma1_winter_vq)./1e3,[v200 v200],'k--','LineWidth',0.8);
% %dmax 2012
% [Cdmax,~]=contour(xx_fine,yy_fine,(squeeze(n2012_allfour.sigma1_39mu(i,:,:))-princ_sigma1_winter_vq)./1e3,[v200 v200]); 
% Cdmax(Cdmax > 15) = NaN;
% NLdmax2012_mu39 = nanmax(sqrt((Cdmax(1,2:end).^2)+(Cdmax(2,2:end).^2)))  
% 
% % %dmax 2012 inland 39
% [Cdmax,~]=contour(xx_fine,yy_fine,(squeeze(n2012_allfour.sigma1_39mu(i,:,:))-princ_sigma1_winter_vq)./1e3,[v200 v200]); 
% Cdmax(Cdmax > 15) = NaN;
% test = Cdmax(1,:); test(test < 0.5) = NaN;
% NLdmax2012_mu39_inland = nanmax(sqrt((test(1,2:end).^2)+(Cdmax(2,2:end).^2))) 
% % 
% contour(xx_fine,yy_fine,(squeeze(n2012_allfour.sigma1_032mu(i,:,:))-princ_sigma1_winter_vq)./1e3,[v200 v200],'w','LineWidth',1.5);
% [Cdmax,~]=contour(xx_fine,yy_fine,(squeeze(n2012_allfour.sigma1_032mu(i,:,:))-princ_sigma1_winter_vq)./1e3,[v200 v200]); 
% Cdmax(Cdmax > 15) = NaN;
% NLdmax2012_mu032 = nanmax(sqrt((Cdmax(1,2:end).^2)+(Cdmax(2,2:end).^2)))
% 
% % %dmax 2012 inland
% [Cdmax,~]=contour(xx_fine,yy_fine,(squeeze(n2012_allfour.sigma1_032mu(i,:,:))-princ_sigma1_winter_vq)./1e3,[v200 v200]); 
% Cdmax(Cdmax > 15) = NaN;
% test = Cdmax(1,:); test(test < 0.5) = NaN;
% NLdmax2012_mu032_inland = nanmax(sqrt((test(1,2:end).^2)+(Cdmax(2,2:end).^2))) 
% 
% % save for plotting values on fig for idealized simulations
% NLdmax2012.mu15 = NLdmax2012_mu15;
% NLdmax2012.mu39 = NLdmax2012_mu39;
% NLdmax2012.mu032 = NLdmax2012_mu032;
% NLdmax2012.mu15_inland = NLdmax2012_mu15_inland;
% NLdmax2012.mu032_inland = NLdmax2012_mu032_inland;
% NLdmax2012.mu39_inland = NLdmax2012_mu39_inland;
% save NLdmax2012_200kPa_surfC_20230724.mat NLdmax2012

% lakes
plot(xy_sta_12(:,1),xy_sta_12(:,2),'k^','MarkerSize',TriangleSize-2,'MarkerFaceColor','none'); hold on;
scatter(xy_lake(:,1),xy_lake(:,2),4,'bo','filled','MarkerEdgeColor','none')
scatter(xy_lil_lake(:,1),xy_lil_lake(:,2),4,'bo','filled','MarkerEdgeColor','none')
scatter(xy_nnl_lake(:,1),xy_nnl_lake(:,2),4,'bo','filled','MarkerEdgeColor','none')
rectangle('Position',[-10 -9.7 20 20],'EdgeColor',[0.7 0.7 0.7],...
...
xlim([-15 15]); ylim([-15 15]); caxis([-1000 1000])
colormap(axe32,BWR); 
title('\sigma_{1,lake}(2012:{\itt}_{3})','FontSize',m+2,'FontName','Avenir');
...
hold on;
[C5,h6]=contourf(xx_fine,yy_fine,(squeeze(n2012_allfour.sigma2(i,:,:))-princ_sigma2_winter_vq)./1e3,v);
set(h6,'LineColor','none'); 
contour(xx_fine,yy_fine,(squeeze(n2012_allfour.sigma2(i,:,:))-princ_sigma2_winter_vq)./1e3,[v200 v200],'k','LineWidth',1.1);
contour(xx_fine,yy_fine,(squeeze(n2012_allfour.sigma2_39mu(i,:,:))-princ_sigma2_winter_vq)./1e3,[v200 v200],'k--','LineWidth',0.8);
contour(xx_fine,yy_fine,(squeeze(n2012_allfour.sigma2_032mu(i,:,:))-princ_sigma2_winter_vq)./1e3,[v200 v200],'w','LineWidth',1.5);
% lakes
plot(xy_sta_12(:,1),xy_sta_12(:,2),'k^','MarkerSize',TriangleSize-2,'MarkerFaceColor','k'); hold on;
scatter(xy_lake(:,1),xy_lake(:,2),4,'bo','filled','MarkerEdgeColor','none')
scatter(xy_lil_lake(:,1),xy_lil_lake(:,2),4,'bo','filled','MarkerEdgeColor','none')
...
text(13.5,-14.05,'N','FontSize',10)
xlim([-15 15]); ylim([-15 15]); caxis([-1000 1000]);
title('\sigma_{2,lake}(2012:{\itt}_{3})','FontSize',m+2,'FontName','Avenir');
colormap(axe52,BWR); 
...
hold on; 
[~,h6]=contourf(xx_fine,yy_fine,(squeeze(n2012_allfour.sigma1(i,:,:)))./1e3,v);
set(h6,'LineColor','none'); 
contour(xx_fine,yy_fine,(squeeze(n2012_allfour.sigma1(i,:,:)))./1e3,[v200 v200],'k','LineWidth',1.1);
contour(xx_fine,yy_fine,(squeeze(n2012_allfour.sigma1_39mu(i,:,:)))./1e3,[v200 v200],'k--','LineWidth',0.8);
contour(xx_fine,yy_fine,(squeeze(n2012_allfour.sigma1_032mu(i,:,:)))./1e3,[v200 v200],'w','LineWidth',1.5);
% lakes
plot(xy_sta_12(:,1),xy_sta_12(:,2),'k^','MarkerSize',TriangleSize-2,'MarkerFaceColor','none'); hold on;
scatter(xy_lake(:,1),xy_lake(:,2),4,'bo','filled','MarkerEdgeColor','none')
scatter(xy_lil_lake(:,1),xy_lil_lake(:,2),4,'bo','filled','MarkerEdgeColor','none')
...
xlim([-15 15]); ylim([-15 15]); caxis([-1000 1000])
colormap(axe33,BWR); 
title('\sigma_{1}(2012:{\itt}_{3})','FontSize',m+2,'FontName','Avenir'); 
set(gca,'xtick',[-10:5:20],'ytick',[-20:5:20],'tickdir','in','LineWidth',1.1,'FontSize',m,...
...
axes(axe53);
hold on; 
contour(xx_fine,yy_fine,(squeeze(n2012_allfour.sigma2_032mu(i,:,:)))./1e3,[v200 v200],'w','LineWidth',1.8);
contour(xx_fine,yy_fine,(squeeze(n2012_allfour.sigma2(i,:,:)))./1e3,[v200 v200],'k','LineWidth',1.2);
contour(xx_fine,yy_fine,(squeeze(n2012_allfour.sigma2_39mu(i,:,:)))./1e3,[v200 v200],'k--','LineWidth',1.2);
[~,h6]=contourf(xx_fine,yy_fine,(squeeze(n2012_allfour.sigma2(i,:,:)))./1e3,v);
set(h6,'LineColor','none'); 
contour(xx_fine,yy_fine,(squeeze(n2012_allfour.sigma2(i,:,:)))./1e3,[v200 v200],'k','LineWidth',1.1);
contour(xx_fine,yy_fine,(squeeze(n2012_allfour.sigma2_39mu(i,:,:)))./1e3,[v200 v200],'k--','LineWidth',0.8);
contour(xx_fine,yy_fine,(squeeze(n2012_allfour.sigma2_032mu(i,:,:)))./1e3,[v200 v200],'w','LineWidth',1.5);
...
xlim([-15 15]); ylim([-15 15]); caxis([-1000 1000]);
title('\sigma_{2}(2012:{\itt}_{3})','FontSize',m+2,'FontName','Avenir');
colormap(axe53,BWR); 
...
% %
%figurename=sprintf('paperfigS1_nif2012_max20kmB_200kPa_principals_surfC_20230724.png');
