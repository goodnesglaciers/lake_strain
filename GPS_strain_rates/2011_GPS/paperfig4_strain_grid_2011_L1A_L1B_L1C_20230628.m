...
clear all; close all;
load('S_neu_nif_2011.mat'); % load TRACK NEU position archive
...
% work through stations 1–14; avoid later portions of the timeseries
AA = horzcat(Q(1).time_new2(4000:25000), smooth(Q(1).e12(4000:25000),span),...
   smooth(Q(1).n12(4000:25000),span),...
   smooth(Q(1).u12(4000:25000),span) );
AA1 = sortrows(AA,1);
...
for i=2:1:4
    Q(i,1).e12i=interp1(Q(i).time_new2,smooth(Q(i).e12,span),xi12);
    Q(i,1).n12i=interp1(Q(i).time_new2,smooth(Q(i).n12,span),yi12);
    Q(i,1).u12i=interp1(Q(i).time_new2,smooth(Q(i).u12,span),zi12);
end

AA = horzcat(Q(5).time_new2(4000:25000), smooth(Q(5).e12(4000:25000),span),...
   smooth(Q(5).n12(4000:25000),span),...
   smooth(Q(5).u12(4000:25000),span) );
AA1 = sortrows(AA,1);
F=griddedInterpolant(AA1(:,1),AA1(:,2));
Q(5).e12i = F(xi12);
F=griddedInterpolant(AA1(:,1),AA1(:,3));
...
AA = horzcat(Q(6).time_new2(4000:25000), smooth(Q(6).e12(4000:25000),span),...
   smooth(Q(6).n12(4000:25000),span),...
   smooth(Q(6).u12(4000:25000),span) );
AA1 = sortrows(AA,1);
F=griddedInterpolant(AA1(:,1),AA1(:,2));
Q(6).e12i = F(xi12);
F=griddedInterpolant(AA1(:,1),AA1(:,3));
...
AA = horzcat(Q(8).time_new2(4000:25000), smooth(Q(8).e12(4000:25000),span),...
   smooth(Q(8).n12(4000:25000),span),...
   smooth(Q(8).u12(4000:25000),span) );
AA1 = sortrows(AA,1);
F=griddedInterpolant(AA1(:,1),AA1(:,2));
Q(8).e12i = F(xi12);
F=griddedInterpolant(AA1(:,1),AA1(:,3));
...
AA = horzcat(Q(9).time_new2(4000:25000), smooth(Q(9).e12(4000:25000),span),...
   smooth(Q(9).n12(4000:25000),span),...
   smooth(Q(9).u12(4000:25000),span) );
AA1 = sortrows(AA,1);
F=griddedInterpolant(AA1(:,1),AA1(:,2));
Q(9).e12i = F(xi12);
F=griddedInterpolant(AA1(:,1),AA1(:,3));
...
AA = horzcat(Q(12).time_new2(4000:25000), smooth(Q(12).e12(4000:25000),span),...
   smooth(Q(12).n12(4000:25000),span),...
   smooth(Q(12).u12(4000:25000),span) );
AA1 = sortrows(AA,1);
F=griddedInterpolant(AA1(:,1),AA1(:,2));
Q(12).e12i = F(xi12);
F=griddedInterpolant(AA1(:,1),AA1(:,3));
...
% %% ERRORS IN STRAIN RATES -- incorrect from LAS
% delta = 0.02; % assume all Track errors are 2 cm
% delta_dx = sqrt(4*(delta^2));
% delta_dy = delta_dx; 
% 
% % this is all in units of day^{-1} 
% for i=1:1:nfQ
%     for j=1:1:nfQ
%         for t=1:length_xi12-2
%            % delta_du and delta_dv
%            delta_du(i,j,t) = abs(du12(i,j,t+1)).*sqrt( (delta/dx12(i,j,t+1)).^2 + (delta/dx12(i,j,t)).^2 +...
%                             (delta/dx12(i,j,t+1))^2 + (delta/dx12(i,j,t))^2 );
%            delta_dv(i,j,t) = abs(dv12(i,j,t+1)).*sqrt( (delta/dy12(i,j,t+1)).^2 + (delta/dy12(i,j,t)).^2 +...
%                             (delta/dy12(i,j,t+1)).^2 + (delta/dy12(i,j,t)).^2 );
%            % delta_Exx             
%            delta_Exx(i,j,t) = abs(dudx(i,j,t+1)).*sqrt( delta_du(i,j,t).^2 + delta_dx^2 );
%            % delta_Eyy             
%            delta_Eyy(i,j,t) = abs(dvdy(i,j,t+1)).*sqrt( delta_dv(i,j,t).^2 + delta_dy^2 );
%            % delta_Elon
%            delta_Elon(i,j,t) = sqrt( 3.*(delta_Exx(i,j,t).^2) + 3.*(delta_Eyy(i,j,t).^2) );
%            
%         end
%     end
% end
% delta_Etrans = delta_Elon;
% 
% % change to units of year^{-1}
% delta_Elon_yr = delta_Elon.*365.25;
% delta_Etrans_yr = delta_Etrans.*365.25;
    ...
    for j=1:1:nfQ
        for t=1:length_xi12-2
           ...
           delta_Exx_yr(i,j,t) = abs(lon_yr(i,j,t)).*sqrt(((1./(du12_f_year(i,j,t).^2)).*...
               (4*delta_sqr))+((1./(dx12_f(i,j,t).^2)).*(2*delta_sqr)));
           % delta_Eyy             
           delta_Eyy_yr(i,j,t) = abs(trans_yr(i,j,t)).*sqrt(((1./(dv12_f_year(i,j,t).^2)).*...
               (4*delta_sqr))+((1./(dy12_f(i,j,t).^2)).*(2*delta_sqr)));
           % delta_Exy
...
h4=plot([171.85 168.85], [-100 100],'-'); c4 = get(h4,'Color');
h5=plot([172.85 168.85], [-100 100],'-'); c5 = get(h5,'Color');
h6=plot([173.85 168.85], [-100 100],'-'); c6 = get(h6,'Color');
h7=plot([174.85 168.85], [-100 100],'-'); c7 = get(h7,'Color');
h8=plot([175.85 168.85], [-100 100],'-'); c8 = get(h8,'Color');
h9=plot([176.85 168.85], [-100 100],'-'); c9 = get(h9,'Color');
...
set(gca,'xtick',[168.5,169,169.5,170]);
ylabel('$\dot{\epsilon}_{xx}\  [\ yr^{-1}\ ]$', 'Interpreter','latex','FontSize',16,'FontName','Avenir');
xlabel(' Day of Year, 2011 ','FontName','Avenir','FontSize',12);
...
ylim([0 0.00004]);
xlim([168.32 170.32]);
text(168.35, 3.6*(10^-5), 'd. L1C','FontSize',12,'FontWeight','bold'); 
...
text(168.85-0.03,0.000043,'\it{t}_{1}','Rotation',0,'FontSize',11,'FontName','Avenir'); 
text(169.21-0.03,0.000043,'\it{t}_{2}','Rotation',0,'FontSize',11,'FontName','Avenir'); 
text(169.32-0.03,0.000043,'\it{t}_{3}','FontSize',11,'FontName','Avenir'); 
text(170.32-0.03,0.000043,'\it{t}_{4}','Rotation',0,'FontSize',11,'FontName','Avenir'); 
...
ylim([0 0.00004]);
xlim([168.32 170.32]);
text(168.35, 3.6*(10^-5), 'e. L1A','FontSize',12,'FontWeight','bold'); 
...
ylim([0 0.00004]);  
xlim([168.32 170.32]);
text(168.35, 3.6*(10^-5), 'f. L1B','FontSize',12,'FontWeight','bold'); 
set(gca,'tickdir','in','LineWidth',1.1,'FontSize',12,'ytick',[0:0.00002:0.0001],'FontName','Avenir'); grid on
set(gca,'xtick',[168.5,169,169.5,170]);
ylabel('${\delta}\dot{\epsilon}_{xx}\  [\ yr^{-1}\ ]$', 'Interpreter','latex','FontSize',16);
xlabel(' Day of Year, 2011 ','FontSize',12); 
...
ylim([0 0.00004]);
xlim([168.32 170.32]);
text(168.35, 3.6*(10^-5), 'g. L1C','FontSize',12,'FontWeight','bold')
text(168.85-0.03,0.000043,'\it{t}_{1}','Rotation',0,'FontSize',11,'FontName','Avenir'); 
text(169.21-0.03,0.000043,'\it{t}_{2}','Rotation',0,'FontSize',11,'FontName','Avenir'); 
text(169.32-0.03,0.000043,'\it{t}_{3}','FontSize',11,'FontName','Avenir'); 
text(170.32-0.03,0.000043,'\it{t}_{4}','Rotation',0,'FontSize',11,'FontName','Avenir'); 
...
ylim([0 0.00004]); 
xlim([168.32 170.32]);
text(168.35, 3.6*(10^-5), 'h. L1A','FontSize',12,'FontWeight','bold')
set(gca,'tickdir','in','LineWidth',1.1,'FontSize',12,'ytick',[0:0.00002:0.0001],'FontName','Avenir'); grid on
set(gca,'xtick',[168.5,169,169.5,170]);
ylabel('${\delta}\dot{\epsilon}_{yy}\  [\ yr^{-1}\ ]$', 'Interpreter','latex','FontSize',16);
...
ylim([0 0.00004]); 
xlim([168.32 170.32]); 
text(168.35, 3.6*(10^-5), 'i. L1B','FontSize',12,'FontWeight','bold')
set(gca,'tickdir','in','LineWidth',1.1,'FontSize',12,'ytick',[0:0.00002:0.0001],'FontName','Avenir'); grid on
set(gca,'xtick',[168.5,169,169.5,170]);
ylabel('${\delta}\dot{\epsilon}_{yy}\  [\ yr^{-1}\ ]$', 'Interpreter','latex','FontSize',16);
xlabel(' Day of Year, 2011 ','FontSize',12);  grid on;

%print(gcf,'-dpng','-r500','paperfigS2_strainrate_errors_2011_20230628.png');
...
plot([xy_sta_11(4,1) xy_sta_11(9,1)],[xy_sta_11(4,2) xy_sta_11(9,2)],'-','Color',c5,'LineWidth',1.5) % nl2 nl8
...
ylim([-0.22 0.4]); 
xlim([168.32 170.32]);
text(168.33, 0.44, '{\bf d. L1A-L1C} (Compare with Fig. 3d.)','FontSize',12); 
...
text(168.33, 0.44, '{\bf e. L1A} (Same as Fig. 3e.)','FontSize',12); 
set(gca,'tickdir','in','LineWidth',1.1,'FontSize',12,'ytick',[-0.2:0.1:0.4],'FontName','Avenir'); grid on;
set(gca,'xtick',[168.5,169,169.5,170]);
ylabel('$\dot{\epsilon}_{xx}\  [\ yr^{-1}\ ]$', 'Interpreter','latex','FontSize',16,'FontName','Avenir');
...
ylim([-0.22 0.4]); 
xlim([168.32 170.32]);
text(168.33, 0.44, '{\bf f. L1A-L1B} (Compare with Fig. 3f)','FontSize',12)
set(gca,'tickdir','in','LineWidth',1.1,'FontSize',12,'ytick',[-0.6:0.1:0.6],'FontName','Avenir');
set(gca,'xtick',[168.5,169,169.5,170]);
ylabel('$\dot{\epsilon}_{xx}\  [\ yr^{-1}\ ]$', 'Interpreter','latex','FontSize',16,'FontName','Avenir');
xlabel(' Day of Year, 2011 ','FontSize',12,'FontName','Avenir');  grid on;
...
text(168.33, 0.44, '{\bf g. L1A-L1C} (Compare with Fig. 3g.)','FontSize',12)
set(gca,'tickdir','in','LineWidth',1.1,'FontSize',12,'ytick',[-0.2:0.1:0.6],'FontName','Avenir'); grid on;
set(gca,'xtick',[168.5,169,169.5,170]);
ylabel('$\dot{\epsilon}_{yy}\  [\ yr^{-1}\ ]$', 'Interpreter','latex','FontSize',16,'FontName','Avenir');
...
text(168.33, 0.44, '{\bf h. L1A} (Same as Fig. 3h.)','FontSize',12) 
set(gca,'tickdir','in','LineWidth',1.1,'FontSize',12,'ytick',[-0.2:0.1:0.6],'FontName','Avenir'); grid on;
set(gca,'xtick',[168.5,169,169.5,170]);
ylabel('$\dot{\epsilon}_{yy}\  [\ yr^{-1}\ ]$', 'Interpreter','latex','FontSize',16,'FontName','Avenir');
...
ylim([-0.22 0.4]); 
xlim([168.32 170.32]);
text(168.33, 0.44, '{\bf i. L1A-L1B}  (Compare with Fig. 3i.)','FontSize',12)
set(gca,'tickdir','in','LineWidth',1.1,'FontSize',12,'ytick',[-0.6:0.1:0.6],'FontName','Avenir');
set(gca,'xtick',[168.5,169,169.5,170]);
ylabel('$\dot{\epsilon}_{yy}\  [\ yr^{-1}\ ]$', 'Interpreter','latex','FontSize',16,'FontName','Avenir');
xlabel(' Day of Year, 2011 ','FontSize',12,'FontName','Avenir');  grid on;

%print(gcf,'-dpng','-r500','paperfigS4_strainrate_twolakebasins_2011_20230628.png');