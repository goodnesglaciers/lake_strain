...
void ReadSinexData(double **d, double **dcov, double **epoch, char **sites, char **types, int n, FILE *fIn) {
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (nrhs != 1 || nlhs>5) {
        mexPrintf("\nread_sinex, v3.0. Reads sinex files.\n\n");
        mexPrintf("Usage: [d,dcov,epoch,sites,types]=read_sinex('filename')\n\n");
        return;
    }
    ...
    ReadSinexData(d, dcov, epoch, sites, types, n, fIn);
