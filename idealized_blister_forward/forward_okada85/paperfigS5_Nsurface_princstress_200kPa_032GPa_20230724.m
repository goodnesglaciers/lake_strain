...
% 30 June 2023 LAS -- finalized for revisions; \mu = 0.32 GPa
clear all; close all;
...
% load saved stresses
load sigma_ij_500m_032mu.mat	
load sigma_ij_750m_032mu.mat
load sigma_ij_1000m_032mu.mat
load sigma_ij_1250m_032mu.mat
load sigma_ij_1500m_032mu.mat
load sigma_ij_1750m_032mu.mat
load sigma_ij_2000m_032mu.mat
load sigma_ij_2250m_032mu.mat
load sigma_ij_2500m_032mu.mat
load sigma_ij_2750m_032mu.mat
load sigma_ij_3000m_032mu.mat
...
for i=2
     % dmax: open + slip
        ...
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax(1,i) = nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2)));
        [C,~]=contour(xx,yy,reshape(((princ_sigma1_750m(i,:))./1e3),121,121),[v200 v200]); C(C > 15) = NaN;
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax(2,i) = nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2)));        
        [C,~]=contour(xx,yy,reshape(((princ_sigma1_1000m(i,:))./1e3),121,121),[v200 v200]); C(C > 15) = NaN;
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax(3,i) = nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2)))
        [C,~]=contour(xx,yy,reshape(((princ_sigma1_1250m(i,:))./1e3),121,121),[v200 v200]); C(C > 15) = NaN;
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax(4,i) =  0; %nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2))) 
        [C,~]=contour(xx,yy,reshape(((princ_sigma1_1500m(i,:))./1e3),121,121),[v200 v200]); C(C > 15) = NaN;
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax(5,i) =  0; %nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2)));
        ...
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax_open(2,i) =  0; %nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2)));        
        [C,~]=contour(xx,yy,reshape(((princ_sigma1_1000m_open(i,:))./1e3),121,121),[v200 v200]); C(C > 15) = NaN;
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax_open(3,i) = nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2)))
        [C,~]=contour(xx,yy,reshape(((princ_sigma1_1250m_open(i,:))./1e3),121,121),[v200 v200]); C(C > 15) = NaN;
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax_open(4,i) =  0; %nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2))) 
        [C,~]=contour(xx,yy,reshape(((princ_sigma1_1500m_open(i,:))./1e3),121,121),[v200 v200]); C(C > 15) = NaN;
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax_open(5,i) =  0; %nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2)));
...
for i=3
     % dmax: open + slip
        ...
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax(3,i) = nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2)))
        [C,~]=contour(xx,yy,reshape(((princ_sigma1_1250m(i,:))./1e3),121,121),[v200 v200]); C(C > 15) = NaN;
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax(4,i) = nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2))) 
        [C,~]=contour(xx,yy,reshape(((princ_sigma1_1500m(i,:))./1e3),121,121),[v200 v200]); C(C > 15) = NaN;
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax(5,i) =  0; %nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2)));
        [C,~]=contour(xx,yy,reshape(((princ_sigma1_1750m(i,:))./1e3),121,121),[v200 v200]); C(C > 15) = NaN;
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax(6,i) =  0; %nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2)))
        ...
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax_open(3,i) =  0; %nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2)))
        [C,~]=contour(xx,yy,reshape(((princ_sigma1_1250m_open(i,:))./1e3),121,121),[v200 v200]); C(C > 15) = NaN;
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax_open(4,i) = nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2))) 
        [C,~]=contour(xx,yy,reshape(((princ_sigma1_1500m_open(i,:))./1e3),121,121),[v200 v200]); C(C > 15) = NaN;
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax_open(5,i) =  0; %nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2)));
        [C,~]=contour(xx,yy,reshape(((princ_sigma1_1750m_open(i,:))./1e3),121,121),[v200 v200]); C(C > 15) = NaN;
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax_open(6,i) =  0; %nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2)))
...
for i=4
     % dmax: open + slip
        ...
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax(4,i) = nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2))) 
        [C,~]=contour(xx,yy,reshape(((princ_sigma1_1500m(i,:))./1e3),121,121),[v200 v200]); C(C > 15) = NaN;
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax(5,i) = nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2)));
        [C,~]=contour(xx,yy,reshape(((princ_sigma1_1750m(i,:))./1e3),121,121),[v200 v200]); C(C > 15) = NaN;
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax(6,i) = nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2)))
        [C,~]=contour(xx,yy,reshape(((princ_sigma1_2000m(i,:))./1e3),121,121),[v200 v200]); C(C > 15) = NaN;
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax(7,i) = nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2)));
        [C,~]=contour(xx,yy,reshape(((princ_sigma1_2250m(i,:))./1e3),121,121),[v200 v200]); C(C > 15) = NaN;
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax(8,i) = nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2)));
        [C,~]=contour(xx,yy,reshape(((princ_sigma1_2500m(i,:))./1e3),121,121),[v200 v200]); C(C > 15) = NaN;
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax(9,i) =  0; %nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2)));
        [C,~]=contour(xx,yy,reshape(((princ_sigma1_2750m(i,:))./1e3),121,121),[v200 v200]); C(C > 15) = NaN;
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax(10,i) =   0; %nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2)));
        ...
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax_open(6,i) =  0; %nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2)))
        [C,~]=contour(xx,yy,reshape(((princ_sigma1_2000m_open(i,:))./1e3),121,121),[v200 v200]); C(C > 15) = NaN;
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax_open(7,i) =  nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2)));
        [C,~]=contour(xx,yy,reshape(((princ_sigma1_2250m_open(i,:))./1e3),121,121),[v200 v200]); C(C > 15) = NaN;
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax_open(8,i) =  nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2)));
        [C,~]=contour(xx,yy,reshape(((princ_sigma1_2500m_open(i,:))./1e3),121,121),[v200 v200]); C(C > 15) = NaN;
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax_open(9,i) =  0; %nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2)));
        [C,~]=contour(xx,yy,reshape(((princ_sigma1_2750m_open(i,:))./1e3),121,121),[v200 v200]); C(C > 15) = NaN;
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax_open(10,i) =  0; %nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2)));
...
for i=5
     % dmax: open + slip
        ...
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax(8,i) = nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2)));
        [C,~]=contour(xx,yy,reshape(((princ_sigma1_2500m(i,:))./1e3),121,121),[v200 v200]); C(C > 15) = NaN;
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax(9,i) = nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2)));
        [C,~]=contour(xx,yy,reshape(((princ_sigma1_2750m(i,:))./1e3),121,121),[v200 v200]); C(C > 15) = NaN;
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax(10,i) =  nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2)));
        [C,~]=contour(xx,yy,reshape(((princ_sigma1_3000m(i,:))./1e3),121,121),[v200 v200]); C(C > 15) = NaN;
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax(11,i) = nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2)));    
        ...
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax_open(8,i) =   0; %nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2)));
        [C,~]=contour(xx,yy,reshape(((princ_sigma1_2500m_open(i,:))./1e3),121,121),[v200 v200]); C(C > 15) = NaN;
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax_open(9,i) =  nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2)));
        [C,~]=contour(xx,yy,reshape(((princ_sigma1_2750m_open(i,:))./1e3),121,121),[v200 v200]); C(C > 15) = NaN;
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax_open(10,i) = nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2)));
        [C,~]=contour(xx,yy,reshape(((princ_sigma1_3000m_open(i,:))./1e3),121,121),[v200 v200]); C(C > 15) = NaN;
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax_open(11,i) =  0; %nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2)));      
...
dmax(isnan(dmax)) = 0; save dmax_idealized_200kPa_032GPa.mat dmax
dmax_open(isnan(dmax_open)) = 0; save dmax_open_idealized_200kPa_032GPa.mat dmax_open
dmax_slip(isnan(dmax_slip)) = 0; save dmax_slip_idealized_200kPa_032GPa.mat dmax_slip
...
axe5 = axes('Position',[0.07 0.05 tt2 tt2*ss],'Box','on'); 
axe6 = axes('Position',[0.32 0.05 tt2 tt2*ss],'Box','on','yticklabel',[]); 
axe7 = axes('Position',[0.57 0.05 tt2 tt2*ss],'Box','on','yticklabel',[]); 
...
xlabel('x  [ km ]'); ylabel('h  [ m ]'); %ylabel('Cavity Opening [ m ]');
...
hold on;
text(-23.5,20, '\mu = 0.32 GPa','FontSize',m+1, 'FontName','AvenirBold');
[c3,h3]=contourf(xx,yy,reshape(((princ_sigma1_1500m_open(4,:))./1e3),121,121),v); 
...
isf=0;
 ...
 % r_m
[C,h]=contour(xx,yy,reshape(((princ_sigma1_1500m_open(4,:))./1e3),121,121),[v200 v200],'k','LineWidth',1.2);
C(C > 15) = NaN; idx = C(1,:)<=0; C(1,idx)=NaN;
d_plot = sqrt(((C(1,1:end).^2)+(C(2,1:end).^2)));

ylabel('y  [ km ]'); 
text(-14, 13.3, 'V=0.05 km^{3}  H=1500 m  \mu = 0.32 GPa','FontName','Avenir','FontSize',m-1)
...
hold on;
[c3,h3]=contourf(xx,yy,reshape(((princ_sigma1_1500m_slip(4,:))./1e3),121,121),v);  set(h3,'LineColor','none'); 
...
% r_m
[C2,h]=contour(xx,yy,reshape(((princ_sigma1_1500m_slip(4,:))./1e3),121,121),[v200 v200],'k','LineWidth',1.2);
C2(C2 > 15) = NaN; idx = C2(1,:)<=0; C2(1,idx)=NaN;
...
d_plot = sqrt(((C3(1,1:end).^2)+(C3(2,1:end).^2))); 
[I,J] = max(d_plot);
quiver([0], [0], [C3(1,J)], [C3(2,J)],'k','autoscale','off','LineWidth',1.4,'AutoScaleFactor',0.75)
text(7, 3, 'r_{m}','FontName','Avenir')
quiver([6.7],[2.3],[-5.0],[-3.2],'LineWidth',1.2,'Color',[0.5 0.5 0.5],'MaxHeadSize',0.7);
...
set(cc43,'Position',[0.789 0.785 .005 (tt*ss)-0.11],'xtick',[-400 -200 0 200 400],'tickdir','in');
text(23.75,-11.0, '\sigma_{1} [ kPa ]','Rotation',90,'FontSize',m,'FontName','Avenir');


% over lake volume
axes(axe12)
text(-23.5,380,'d','FontWeight','bold','FontSize',m+1);
hold on;
for i=1:5
    this = reshape(princ_sigma1_1500m_open(i,:),121,121);
    transect_open(i,:) = this(62,:);
end
...
plot(xx(42,:), transect_open(1,:)./1e3,'LineWidth',1.2,'Color',[217 240 163]./250); % 0.01

ylabel('\sigma_{1} [ kPa ]');

text(-14, -65, 'H=1500 m                     \mu = 0.32 GPa','FontName','Avenir','FontSize',m-1)
text(-14, 225, '200 kPa','FontName','Avenir')
xlim([-15 15]); ylim([-100 400]);
set(gca,'tickdir','in','xtick',[-15:5:15],'ytick',[-100:100:400],'FontSize',m,'FontName','Avenir','LineWidth',0.8);
grid on 

axes(axe22)
text(-18.5,380,'e','FontWeight','bold','FontSize',m+1);
hold on;
for i=1:5
    this = reshape(princ_sigma1_1500m_slip(i,:),121,121);
    transect_slip(i,:) = this(62,:);
end
...
plot(xx(42,:), transect_slip(1,:)./1e3,'LineWidth',1.2,'Color',[217 240 163]./250); % 0.01
...
axes(axe32)
text(-18,380,'f','FontWeight','bold','FontSize',m+1);
hold on;
for i=1:5
...
plot(xx(42,:), transect(1,:)./1e3,'LineWidth',1.2,'Color',[217 240 163]./250); % 0.01
plot([-20 20],[200 200],'-','LineWidth',1.2,'Color',[0.6 0.6 0.6]);
plot(xx(42,:), transect(5,:)./1e3,'LineWidth',1.2,'Color',[0 104 55]./250); % 0.001
plot(xx(42,:), transect(4,:)./1e3,'LineWidth',1.2,'Color',[49 163 84]./250);
plot(xx(42,:), transect(3,:)./1e3,'LineWidth',1.2,'Color',[120 198 121]./250);
...
text(18,220,'Lake Volume:','FontSize',m,'FontName','Avenir')

% over ice thickness
axes(axe13)
text(-23.5,380,'g','FontWeight','bold','FontSize',m+1);
hold on;
for i=4
    ...
    transect_thick_open = vertcat(this1(62,:),this15(62,:),this2(62,:),this25(62,:),this3(62,:));
end

plot([-20 20],[200 200],'-','LineWidth',1.2,'Color',[0.6 0.6 0.6]);
plot(xx(52,:), this051(62,:)./1e3,'LineWidth',1.2,'Color',[8 81 156]./250); % 0.001
...
plot(xx(42,:), transect_thick_open(5,:)./1e3,'LineWidth',1.2,'Color',[198 219 239]./250); % 0.01
...
ha = area([bound_pos bound_neg]./1e3, [4000 4000],'FaceColor',[0.6 0.6 0.6],'EdgeColor','none','FaceAlpha',0.2);
...
text(-4, 420, 'Blister Width','FontSize',m-2,'FontName','Avenir')
plot([bound_neg bound_pos]./1e3,[375 375],'k.-','Linewidth',1.2,'MarkerSize',10)
text(-14, -65, 'V=0.05 km^{3}                     \mu = 0.32 GPa','FontName','Avenir','FontSize',m-1)
text(-14, 225, '200 kPa','FontName','Avenir')
xlim([-15 15]); ylim([-100 400]); xlabel('x  [ km ]');
set(gca,'tickdir','in','xtick',[-15:5:15],'ytick',[-100:100:400],'FontSize',m,'FontName','Avenir','LineWidth',0.8);
grid on 

axes(axe23)
text(-18.5,380,'h','FontWeight','bold','FontSize',m+1);
hold on;
for i=4
    ...
    transect_thick_slip = vertcat(this1(62,:),this15(62,:),this2(62,:),this25(62,:),this3(62,:));
end

plot([-20 20],[200 200],'-','LineWidth',1.2,'Color',[0.6 0.6 0.6]);
plot(xx(52,:), this052(62,:)./1e3,'LineWidth',1.2,'Color',[8 81 156]./250); % 0.001
...
plot(xx(42,:), transect_thick_slip(4,:)./1e3,'LineWidth',1.2,'Color',[198 219 239]./250); % 0.01
plot(xx(42,:), transect_thick_slip(5,:)./1e3,'LineWidth',1.2,'Color',[198 219 239]./250); % 0.01
...
text(-5, 420, 'Slip Patch Width','FontSize',m-2,'FontName','Avenir')
plot([bound_neg bound_pos]./1e3,[375 375],'k.-','Linewidth',1.2,'MarkerSize',10)
text(-14, -65, 'V=0.05 km^{3}                     \mu = 0.32 GPa','FontName','Avenir','FontSize',m-1)
text(-14, 225, '200 kPa','FontName','Avenir')
xlim([-15 15]); ylim([-100 400]);; xlabel('x  [ km ]');
...
axes(axe33)
text(-18,380,'i','FontWeight','bold','FontSize',m+1);
hold on;
for i=4
    ...
    transect_thick = vertcat(this1(62,:),this15(62,:),this2(62,:),this25(62,:),this3(62,:));
end

plot(xx(52,:), this053(62,:)./1e3,'LineWidth',1.2,'Color',[8 81 156]./250); % 1000
...
plot(xx(42,:), transect_thick(5,:)./1e3,'LineWidth',1.2,'Color',[198 219 239]./250); % 0.01
plot([-20 20],[200 200],'-','LineWidth',1.2,'Color',[0.6 0.6 0.6]);
...
text(-9, 420, 'Blister and Slip Patch Width','FontSize',m-2,'FontName','Avenir')
plot([bound_neg bound_pos]./1e3,[375 375],'k.-','Linewidth',1.2,'MarkerSize',10)
text(-14, -65, 'V=0.05 km^{3}                     \mu = 0.32 GPa','FontName','Avenir','FontSize',m-1)
text(-14, 225, '200 kPa','FontName','Avenir')
xlim([-15 15]); ylim([-100 400]); xlabel('x  [ km ]'); 
...
text(18,220,'Ice Thickness:','FontSize',m,'FontName','Avenir')

axes(axe5)
text(-150,5.65,'j','FontWeight','bold','FontSize',m+1);
hold on;
title('\sigma_{1,bedopen}');
...
xlim([500 3000]); ylim([0 6]);
set(gca,'xtick',[500:500:3000],'ytick',[0:2:6],'tickdir','in','FontSize',m,'FontName','Avenir','LineWidth',0.8)
ylabel('r_{m}(\sigma_{1}>200 kPa)  [ km ]'); 
xlabel('Ice Thickness, H  [ m ]');
text(2100, 5.25, '\mu = 0.32 GPa','FontName','Avenir','FontSize',m-1)
grid on 

axes(axe6)
text(220,5.65,'k','FontWeight','bold','FontSize',m+1);
hold on;
title('\sigma_{1,bedslip}');
...
axes(axe7)
text(220,5.65,'l','FontWeight','bold','FontSize',m+1);
hold on;
title('\sigma_{1,bedopen+bedslip}');
...
% NL 2011 and 2012
load NLdmax2011_surfC_200kPa_20230721.mat
load NLdmax2012_200kPa_surfC_20230724.mat
% L1A center is at [1.5,0.5] in map space
% dock at eel pond 
...
plot(980,NLdmax2011_surfC.mu032_inland-1.5,...
    'sb','LineWidth',0.9,'MarkerFaceColor',handle,'Color',handle,'MarkerSize',5);
plot(980,NLdmax2012.mu032_inland-1.5,...
...
lgd = legend('0.1 km^{3}','0.05 km^{3}','0.01 km^{3}','0.005 km^{3}','0.001 km^{3}',...
    'Cases in Panels a-c','L1A 2011 (V=0.008 km^{3})');
legend boxoff;
set(lgd,'Position',[0.83 0.075 0.1 0.1],'FontSize',m-1);
text(3200,4.75,'Lake Volume:','FontSize',m,'FontName','Avenir')

%% print figure
% figurename=sprintf('paperfigS5_idealized_200kPa_032GPa_20230724.png');
