...
% 2023 July 20 LAS -- check with curved vertical HF crack, opening of 0.5 m
% 2023 July 20 LAS -- check with curved vertical HF crack, opening
% distribution for vert patches = 2012 t_{3} NIF solution
close all; clear all;

%% load saved displacement, strain, stress, and idealized patches
load surface_disp_1000m_EWcurve_2012_t3.mat
load Gsurface1000m_strain_EWcurve.mat
load sigma_ij_1000m_EWcurve_2012_t3.mat
...
% bed open and slip 
        princ_sigma1_1000m(i,j) = (0.5.*(sigma_ij_1000m_SLder.sigma_xx(i,j)+sigma_ij_1000m_SLder.sigma_yy(i,j))) + ...
            (sqrt(((0.5.*(sigma_ij_1000m_SLder.sigma_xx(i,j)-sigma_ij_1000m_SLder.sigma_yy(i,j))).^2) + ...
            (sigma_ij_1000m_SLder.sigma_xy(i,j).^2)));            
% bed open 
        princ_sigma1_1000m_open(i,j) = (0.5.*(sigma_ij_1000m_SLder.sigma_xx_open(i,j)+sigma_ij_1000m_SLder.sigma_yy_open(i,j))) + ...
            (sqrt(((0.5.*(sigma_ij_1000m_SLder.sigma_xx_open(i,j)-sigma_ij_1000m_SLder.sigma_yy_open(i,j))).^2) + ...
            (sigma_ij_1000m_SLder.sigma_xy_open(i,j).^2)));   
% bed slip 
        princ_sigma1_1000m_slip(i,j) = (0.5.*(sigma_ij_1000m_SLder.sigma_xx_slip(i,j)+sigma_ij_1000m_SLder.sigma_yy_slip(i,j))) + ...
            (sqrt(((0.5.*(sigma_ij_1000m_SLder.sigma_xx_slip(i,j)-sigma_ij_1000m_SLder.sigma_yy_slip(i,j))).^2) + ...
            (sigma_ij_1000m_SLder.sigma_xy_slip(i,j).^2)));   
% vertical crack open 
        princ_sigma1_1000m_vert(i,j) = (0.5.*(sigma_ij_1000m_SLder.sigma_xx_vert(i,j)+sigma_ij_1000m_SLder.sigma_yy_vert(i,j))) + ...
            (sqrt(((0.5.*(sigma_ij_1000m_SLder.sigma_xx_vert(i,j)-sigma_ij_1000m_SLder.sigma_yy_vert(i,j))).^2) + ...
            (sigma_ij_1000m_SLder.sigma_xy_vert(i,j).^2)));   
% bed open, bed slip, vertical crack open 
        princ_sigma1_1000m_vertopenslip(i,j) = (0.5.*(sigma_ij_1000m_SLder.sigma_xx_vertopenslip(i,j)+sigma_ij_1000m_SLder.sigma_yy_vertopenslip(i,j))) + ...
            (sqrt(((0.5.*(sigma_ij_1000m_SLder.sigma_xx_vertopenslip(i,j)-sigma_ij_1000m_SLder.sigma_yy_vertopenslip(i,j))).^2) + ...
            (sigma_ij_1000m_SLder.sigma_xy_vertopenslip(i,j).^2)));  
     end
end
...
%     '-k','LineWidth',3); % single-strike fracture
plot(Gsurface.patchesC(:,6),Gsurface.patchesC(:,7),'-k','LineWidth',3); % multi-strike fracture    
ylabel('y  [ km ]'); 
text(-11, 11, 'Crack Opening = 2012 t_{3} values','FontName','Avenir','FontSize',m-1)
...
text(-11, 11, 'V = 0.01 km^{3}','FontName','Avenir','FontSize',m-1)
...
hold on;
[c3,h3]=contourf(xx,yy,reshape(((surface_disp_1000m.Disp_U_open(5,:))),121,121),v_disp); 
set(h3,'LineColor','none');
FE = interp2(xx_init,yy_init,reshape(surface_disp_1000m.Disp_E_open(5,:),121,121),xx_plot,yy_plot);
FN = interp2(xx_init,yy_init,reshape(surface_disp_1000m.Disp_N_open(5,:),121,121),xx_plot,yy_plot);
...
text(-11, -10.8, 'V = 0.01 km^{3}    H = 1000 m   \mu = 1.5 GPa','FontName','Avenir','FontSize',m-1)
title('u_{bedopen}');
...
hold on;
[c3,h3]=contourf(xx,yy,10.*reshape(((surface_disp_1000m.Disp_U_vert(5,:))),121,121),v_disp); 
set(h3,'LineColor','none');

FE = interp2(xx_init,yy_init,reshape(surface_disp_1000m.Disp_E_vert(5,:),121,121),xx_plot,yy_plot);
FN = interp2(xx_init,yy_init,reshape(surface_disp_1000m.Disp_N_vert(5,:),121,121),xx_plot,yy_plot);
quiver(xx_plot,yy_plot,10.*quiver_scale.*FE,10.*quiver_scale.*FN,'k','autoscale','off','LineWidth',0.6,'Color',BWR(10,:));
quiver(-9.00, 11.1, 10.*quiver_scale.*0.5, 10.*quiver_scale.*0, 'k','autoscale','off','LineWidth',1.0,...
    'Color',BWR(10,:),'MaxHeadSize',0.5);
text(-9, 10, '0.5 m','FontSize',m-1,'Color',BWR(10,:),'FontName','Avenir');
text(-11, -10.8, 'H = 1000 m   \mu = 1.5 GPa','FontName','Avenir','FontSize',m-1)
title('u_{HF} (scaled 10X to see better)'); ylabel('y  [ km ]');
...
hold on;
[c3,h3]=contourf(xx,yy,reshape(((surface_disp_1000m.Disp_U_slip(5,:))),121,121),v_disp); 
set(h3,'LineColor','none'); 

FE = interp2(xx_init,yy_init,reshape(surface_disp_1000m.Disp_E_slip(5,:),121,121),xx_plot,yy_plot);
FN = interp2(xx_init,yy_init,reshape(surface_disp_1000m.Disp_N_slip(5,:),121,121),xx_plot,yy_plot);
quiver(xx_plot,yy_plot,quiver_scale.*FE,quiver_scale.*FN,'k','autoscale','off','LineWidth',0.6,'Color',BWR(10,:));
quiver(-9.00, 11.1, quiver_scale.*1, quiver_scale.*0, 'k','autoscale','off','LineWidth',1.0,...
    'Color',BWR(10,:),'MaxHeadSize',0.8);
text(-9, 10, '1 m','FontSize',m-1,'Color',BWR(10,:),'FontName','Avenir');
...
axes(axe42) % U_{bedopen+bedslip}
text(-12,13.5,'g','FontWeight','bold','FontSize',m+1);
hold on;
[c3,h3]=contourf(xx,yy,reshape(((surface_disp_1000m.Disp_U(5,:))),121,121),v_disp); 
set(h3,'LineColor','none'); 
FE = interp2(xx_init,yy_init,reshape(surface_disp_1000m.Disp_E(5,:),121,121),xx_plot,yy_plot);
FN = interp2(xx_init,yy_init,reshape(surface_disp_1000m.Disp_N(5,:),121,121),xx_plot,yy_plot);
quiver(xx_plot,yy_plot,quiver_scale.*FE,quiver_scale.*FN,'k','autoscale','off','LineWidth',0.6,'Color',BWR(10,:));
quiver(-9.00, 11.1, quiver_scale.*1, quiver_scale.*0, 'k','autoscale','off','LineWidth',1.0,...
    'Color',BWR(10,:),'MaxHeadSize',0.8);
text(-9, 10, '1 m','FontSize',m-1,'Color',BWR(10,:),'FontName','Avenir');
...
axes(axe52) % U_{HF+bedopen+bedslip}
text(-12,13.5,'h','FontWeight','bold','FontSize',m+1);
hold on;
[c3,h3]=contourf(xx,yy,reshape(((surface_disp_1000m.Disp_U_vertopenslip(5,:))),121,121),v_disp); 
set(h3,'LineColor','none'); 

FE = interp2(xx_init,yy_init,reshape(surface_disp_1000m.Disp_E_vertopenslip(5,:),121,121),xx_plot,yy_plot);
FN = interp2(xx_init,yy_init,reshape(surface_disp_1000m.Disp_N_vertopenslip(5,:),121,121),xx_plot,yy_plot);
quiver(xx_plot,yy_plot,quiver_scale.*FE,quiver_scale.*FN,'k','autoscale','off','LineWidth',0.6,'Color',BWR(10,:));
quiver(-9.00, 11.1, quiver_scale.*1, quiver_scale.*0, 'k','autoscale','off','LineWidth',1.0,...
    'Color',BWR(10,:),'MaxHeadSize',0.8);
text(-9, 10, '1 m','FontSize',m-1,'Color',BWR(10,:),'FontName','Avenir');
...
hold on;
[~,h3]=contourf(xx,yy,reshape(((princ_sigma1_1000m_vert(5,:))./1e3),121,121),v); set(h3,'LineColor','none'); 
[C3,~]=contour(xx,yy,reshape(((princ_sigma1_1000m_vert(5,:))./1e3),121,121),[v200 v200],'Color',BWR(10,:),'LineWidth',1.1);
...
hold on;
[~,h3]=contourf(xx,yy,reshape(((princ_sigma1_1000m_open(5,:))./1e3),121,121),v); set(h3,'LineColor','none'); 
[C,~]=contour(xx,yy,reshape(((princ_sigma1_1000m_open(5,:))./1e3),121,121),[v200 v200],'Color',BWR(10,:),'LineWidth',1.1);
text(-11, -10.8, 'V = 0.01 km^{3}    H = 1000 m   \mu = 1.5 GPa','FontName','Avenir','FontSize',m-1)
...
hold on;
[~,h3]=contourf(xx,yy,reshape(((princ_sigma1_1000m_slip(5,:))./1e3),121,121),v);  set(h3,'LineColor','none'); 
[C2,~]=contour(xx,yy,reshape(((princ_sigma1_1000m_slip(5,:))./1e3),121,121),[v200 v200],'Color',BWR(10,:),'LineWidth',1.1);
...
hold on;
[~,h3]=contourf(xx,yy,reshape(((princ_sigma1_1000m(5,:))./1e3),121,121),v); set(h3,'LineColor','none'); 
[C3,~]=contour(xx,yy,reshape(((princ_sigma1_1000m(5,:))./1e3),121,121),[v200 v200],'Color',BWR(10,:),'LineWidth',1.1);
...
hold on;
[~,h3]=contourf(xx,yy,reshape(((princ_sigma1_1000m_vertopenslip(5,:))./1e3),121,121),v); set(h3,'LineColor','none'); 
[C3,~]=contour(xx,yy,reshape(((princ_sigma1_1000m_vertopenslip(5,:))./1e3),121,121),[v200 v200],'Color',BWR(10,:),'LineWidth',1.1);
...
%% print figure
figurename=sprintf('paperfig3_idealized_example_EWcurve_2012_t3_20230720.png');
