...
% 30 June 2023 LAS -- finalized for revisions; \mu = 3.9 GPa
...
% load saved stresses
load sigma_ij_500m_39mu.mat
load sigma_ij_750m_39mu.mat
load sigma_ij_1000m_39mu.mat
load sigma_ij_1500m_39mu.mat
load sigma_ij_1250m_39mu.mat
load sigma_ij_1500m_39mu.mat
load sigma_ij_1750m_39mu.mat
load sigma_ij_2000m_39mu.mat
load sigma_ij_2250m_39mu.mat
load sigma_ij_2500m_39mu.mat
load sigma_ij_2750m_39mu.mat
load sigma_ij_3000m_39mu.mat
...
for i=1
     % dmax: open + slip
        ...
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax_open(1,i) = nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2)));
        [C,~]=contour(xx,yy,reshape(((princ_sigma1_750m_open(i,:))./1e3),121,121),[v200 v200]); C(C > 15) = NaN;
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax_open(2,i) = nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2)));        
        ...
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax_open(5,i) = nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2)));
        [C,~]=contour(xx,yy,reshape(((princ_sigma1_1750m_open(i,:))./1e3),121,121),[v200 v200]); C(C > 15) = NaN;
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax_open(6,i) = nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2)))
...
for i=2
     % dmax: open + slip
        ...
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax_slip(7,i) = nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2)));
        [C,~]=contour(xx,yy,reshape(((princ_sigma1_2250m_slip(i,:))./1e3),121,121),[v200 v200]); C(C > 15) = NaN;
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax_slip(8,i) = nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2)));
        [C,~]=contour(xx,yy,reshape(((princ_sigma1_2500m_slip(i,:))./1e3),121,121),[v200 v200]); C(C > 15) = NaN;
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax_slip(9,i) = nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2)));
        [C,~]=contour(xx,yy,reshape(((princ_sigma1_2750m_slip(i,:))./1e3),121,121),[v200 v200]); C(C > 15) = NaN;
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax_slip(10,i) = 0 %nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2)));
        [C,~]=contour(xx,yy,reshape(((princ_sigma1_3000m_slip(i,:))./1e3),121,121),[v200 v200]); C(C > 15) = NaN;
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax_slip(11,i) = 0; %nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2)));  
...
for i=3
     % dmax: open + slip
        ...
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax_slip(9,i) = nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2)));
        [C,~]=contour(xx,yy,reshape(((princ_sigma1_2750m_slip(i,:))./1e3),121,121),[v200 v200]); C(C > 15) = NaN;
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax_slip(10,i) =nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2)));
        [C,~]=contour(xx,yy,reshape(((princ_sigma1_3000m_slip(i,:))./1e3),121,121),[v200 v200]); C(C > 15) = NaN;
        idx = C(1,:)<=0; C(1,idx)=NaN; dmax_slip(11,i) = nanmax(sqrt((C(1,2:end).^2)+(C(2,2:end).^2)));      
end
...
dmax(isnan(dmax)) = 0; save dmax_idealized_200kPa_39GPa.mat dmax
dmax_open(isnan(dmax_open)) = 0; save dmax_open_idealized_200kPa_39GPa.mat dmax_open
dmax_slip(isnan(dmax_slip)) = 0; save dmax_slip_idealized_200kPa_39GPa.mat dmax_slip
...
hold on;
text(-23.5,20, '\mu = 3.9 GPa','FontSize',m+1, 'FontName','AvenirBold');
...
ylabel('y  [ km ]'); 
text(-14, 13.3, 'V=0.05 km^{3}  H=1500 m  \mu = 3.9 GPa','FontName','Avenir','FontSize',m-1)
title('\sigma_{1,bedopen}');
caxis([-2000 2000]); colormap(axe1,BWR); 
...
C2(C2 > 15) = NaN; idx = C(1,:)<=0; C(1,idx)=NaN;
d_plot2 = sqrt(((C2(1,1:end).^2)+(C2(2,1:end).^2)));
[I,J] = max(d_plot2);
quiver([0], [0], [C2(1,J)], [C2(2,J)],'k','autoscale','off','LineWidth',1.4,'AutoScaleFactor',0.75)
text(11, 5, 'r_{m}','FontName','Avenir')
quiver([10.7],[4.3],[-3.5],[-3.9],'LineWidth',1.2,'Color',[0.5 0.5 0.5],'MaxHeadSize',0.7);

caxis([-2000 2000]);
text(-14, 13.3, 'V=0.05 km^{3}  H=1500 m  \mu = 3.9 GPa','FontName','Avenir','FontSize',m-1)
xlim([-15 15]); ylim([-15 15]);colormap(axe2,BWR); 
set(gca,'tickdir','in','xtick',[-15:5:15],'ytick',[-15:5:15],'FontSize',m,'FontName','Avenir',...
    'LineWidth',0.8,'Layer','top');
grid on
...
quiver([7.2],[7.2],[-6.2],[-3.1],'LineWidth',1.0,'Color',[0.5 0.5 0.5],'MaxHeadSize',0.7);
...
set(cc43,'Position',[0.789 0.785 .005 (tt*ss)-0.11],'xtick',[-2000 -1000 0 1000 2000],'tickdir','in');
text(23.75,-11.0, '\sigma_{1} [ kPa ]','Rotation',90,'FontSize',m,'FontName','Avenir');
%text(-2,-13.8,'\sigma_{1}  [ kPa ]','FontSize',m+1,'FontName','Avenir');

% over lake volume
axes(axe12)
text(-23.5,3600,'d','FontWeight','bold','FontSize',m+1);
hold on;
for i=1:5
...
text(-14, -1450, 'H=1500 m                     \mu = 3.9 GPa','FontName','Avenir','FontSize',m-1)
text(-14, 475, '200 kPa','FontName','Avenir')
xlim([-15 15]); ylim([-2000 4000]);
set(gca,'tickdir','in','xtick',[-15:5:15],'ytick',[-2000:1000:4000],'FontSize',m,'FontName','Avenir','LineWidth',0.8);
grid on 

axes(axe22)
text(-18.5,3600,'e','FontWeight','bold','FontSize',m+1);
hold on;
for i=1:5
...
axes(axe32)
text(-18,3600,'f','FontWeight','bold','FontSize',m+1);
hold on;
for i=1:5
...
text(18,1800,'Lake Volume:','FontSize',m,'FontName','Avenir')

% over ice thickness
axes(axe13)
text(-23.5,3600,'g','FontWeight','bold','FontSize',m+1);
hold on;
for i=4
    this051 = reshape(princ_sigma1_500m_open(i,:),121,121);
    this1 = reshape(princ_sigma1_1500m_open(i,:),121,121);
...
text(-4, 4200, 'Blister Width','FontSize',m-2,'FontName','Avenir')
plot([bound_neg bound_pos]./1e3,[3750 3750],'k.-','Linewidth',1.2,'MarkerSize',10)
text(-14, -1450, 'V=0.05 km^{3}                     \mu = 3.9 GPa','FontName','Avenir','FontSize',m-1)
text(-14, 475, '200 kPa','FontName','Avenir')
xlim([-15 15]); ylim([-2000 4000]); xlabel('x  [ km ]');
set(gca,'tickdir','in','xtick',[-15:5:15],'ytick',[-2000:1000:4000],'FontSize',m,'FontName','Avenir','LineWidth',0.8);
grid on 

axes(axe23)
text(-18.5,3600,'h','FontWeight','bold','FontSize',m+1);
hold on;
for i=4
    this052 = reshape(princ_sigma1_500m_slip(i,:),121,121);
    this1 = reshape(princ_sigma1_1500m_slip(i,:),121,121);
...
text(-5, 4200, 'Slip Patch Width','FontSize',m-2,'FontName','Avenir')
plot([bound_neg bound_pos]./1e3,[3750 3750],'k.-','Linewidth',1.2,'MarkerSize',10)
text(-14, -1450, 'V=0.05 km^{3}                     \mu = 3.9 GPa','FontName','Avenir','FontSize',m-1)
text(-14, 475, '200 kPa','FontName','Avenir')
xlim([-15 15]); ylim([-2000 4000]); xlabel('x  [ km ]');
...
axes(axe33)
text(-18,3600,'i','FontWeight','bold','FontSize',m+1);
hold on;
for i=4
    this053 = reshape(princ_sigma1_500m(i,:),121,121);
    this1 = reshape(princ_sigma1_1500m(i,:),121,121);
...
text(-9, 4200, 'Blister and Slip Patch Width','FontSize',m-2,'FontName','Avenir')
plot([bound_neg bound_pos]./1e3,[3750 3750],'k.-','Linewidth',1.2,'MarkerSize',10)
text(-14, -1450, 'V=0.05 km^{3}                     \mu = 3.9 GPa','FontName','Avenir','FontSize',m-1)
text(-14, 475, '200 kPa','FontName','Avenir')
xlim([-15 15]); ylim([-2000 4000]); xlabel('x  [ km ]');
...
text(18,1800,'Ice Thickness:','FontSize',m,'FontName','Avenir')

axes(axe5)
text(-150,9.5,'j','FontWeight','bold','FontSize',m+1);
hold on;
title('\sigma_{1,bedopen}');
...
xlim([500 3000]); ylim([0 10]);
set(gca,'xtick',[500:500:3000],'ytick',[0:2:10],'tickdir','in','FontSize',m,'FontName','Avenir','LineWidth',0.8)
ylabel('r_{m}(\sigma_{1}>200 kPa)  [ km ]'); 
xlabel('Ice Thickness, H  [ m ]');
text(2100, 9.25, '\mu = 3.9 GPa','FontName','Avenir','FontSize',m-1)
grid on 

axes(axe6)
text(220,9.5,'k','FontWeight','bold','FontSize',m+1);
hold on;
title('\sigma_{1,bedslip}');
...
axes(axe7)
text(220,9.5,'l','FontWeight','bold','FontSize',m+1);
hold on;
...
% NL center is at [1.5,0.5] in map space
% dock at eel pond 
...
plot(980,NLdmax2011_surfC.mu39_inland-1.5,...
    'sb','LineWidth',0.9,'MarkerFaceColor',handle,'Color',handle,'MarkerSize',5);
plot(980,NLdmax2012.mu39_inland-1.5,...
...
text(3200,8.75,'Lake Volume:','FontSize',m,'FontName','Avenir')

%% print figure
% figurename=sprintf('paperfigS6_idealized_200kPa_39GPa_20230724.png');
