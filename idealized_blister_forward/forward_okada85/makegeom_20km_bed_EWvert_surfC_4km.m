function [patchesC, patchesB, G3v, G2bv, G3bv, G3v_strain, G2bv_strain, G3bv_strain]=makegeom_20km_bed_EWvert_surfC_4km(origin,depthc)
path('../software/okada85',path)
%% LAURA: FAULT GEOMOETRY FOR  BED + VERTICAL HYDRO-FRACTURE CRACK
...
buried=1e-6; % buried fault (km); buried vertical HF by 0.001 m to avoid crack intersecting half-space surface
...
%% CONSTANT STRIKE HYDRO-FRACTURE GEOMETRY (LAS 2023 April 10)
Nvpatches = 24; % number of patches along the strike of the H-F 

% make a 4-km-long, E-W crack through the origin
xq = linspace(-2+(2/(Nvpatches+1)),2-(2/(Nvpatches+1)),Nvpatches+1); %% <--- set how many vert planes wanted HERE!!! n=planes desired+1
vq = zeros(1, length(xq));
xy_scarp_interp = horzcat(xq',vq');
 
% % make a 4-km-long, SW-NE crack through the origin
% xq = linspace(-sqrt(8)/2,sqrt(8)/2,Nvpatches+1); %% <--- set how many vert planes wanted HERE!!! n=planes desired+1
% vq = linspace(-sqrt(8)/2,sqrt(8)/2,Nvpatches+1);
% xy_scarp_interp = horzcat(xq',vq');

% % make a 4-km-long, NW-SE crack through the origin
% xq = linspace(-sqrt(8)/2,sqrt(8)/2,Nvpatches+1); %% <--- set how many vert planes wanted HERE!!! n=planes desired+1
% vq = linspace(sqrt(8)/2,-sqrt(8)/2,Nvpatches+1);
% xy_scarp_interp = horzcat(xq',vq');

% new del and segment lengths for continuous crack
...
for i=1:1:Nvpatches
    ...
    disgeom(i,6) = (xy_scarp_interp(i,1)+xy_scarp_interp(i+1,1))/2; % East offset of patch midpoint from origin (km)
    disgeom(i,7) = (xy_scarp_interp(i,2)+xy_scarp_interp(i+1,2))/2; % North offset of patch midpoint from origin (km)
%    disgeom(i,6) = xy_scarp_interp(i,1); % East offset of patch edge from origin (km)
%    disgeom(i,7) = xy_scarp_interp(i,2); % North offset of patch edge from origin (km)
end
